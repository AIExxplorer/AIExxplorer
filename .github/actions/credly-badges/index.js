...
function generateBadgesMarkdown(badgeIds) {
  let markdown = '\n<div align="center" style="display: flex; flex-wrap: wrap; justify-content: center; gap: 20px; margin: 30px 0;">\n';
  
  badgeIds.forEach((badgeId) => {
    const badgeUrl = 'https://www.credly.com/badges/' + badgeId;
    // Use Credly's direct image URL for the badge with transparent background
    const imageUrl = 'https://api.accredible.com/v1/frontend/credential_website_embed_image/' + badgeId;
    
    markdown += '  <a href="' + badgeUrl + '" target="_blank" rel="noopener noreferrer" style="display: inline-block; text-decoration: none;">\n';
    markdown += '    <img src="https://www.credly.com/badges/' + badgeId + '/public_url" alt="Credly Badge" width="100" height="100" style="background-color: transparent; border: none; padding: 5px;" />\n';
    markdown += '  </a>\n';
  });
...
async function main() {
  try {
    ...
    const badgesMarkdown = generateBadgesMarkdown(BADGE_IDS);
    ...
    if (badgesRegex.test(readmeContent)) {
      readmeContent = readmeContent.replace(
        badgesRegex,
        '<!--START_SECTION:badges-->' + badgesMarkdown + '<!--END_SECTION:badges-->'
      );
